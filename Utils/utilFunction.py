...
from Backend.Util.BackendUtilFunctions import calculate_block_matrix_encoding, get_encoded_block_aggregation, get_executable_queries
...
def get_log_lines(file_name, read_from_file, get_queries=False, test_name='') -> List[LogLine]:
    if read_from_file:
        loglines = cPickle.load(open(file_name + '_loglines.p', 'rb'))
        return loglines

    sep = Config.csv_file_separator

    print("./Data/" + file_name + ".txt")
    ...
    if get_queries:
        if 'gtest' in test_name:
            test_name = test_name.replace('gtest', 'test')
        elif 'btest' in test_name:
            test_name = test_name.replace('btest', 'test')
        get_executable_queries(test_name, df) # this is only necessary if the queries have syntax error (mostly because of the qouting char) otherwise ignore it
...
def get_complete_query_result_details3(df, partition_manager: PartitionManager, separateTbs=True, pid_selection_mode='last'):
    df['resultParCount'] = 0
    df['resultTables'] = '-'
    tb_accessed_pid_counter = {}
    for tb in Config.table_list:
        tb_accessed_pid_counter[tb] = []
    for index, row in df.iterrows():
        res_partitions = row['resultPartitions'][1:-1].replace(' ', '').split(',')
        df.at[index, 'resultParCount'] = len(res_partitions)

        res_blocks = row['resultBlock'][1:-1].replace(' ', '').split(',')
        tb_list = [bid.rsplit('_', 1)[0] for bid in res_blocks]
        res_tb_list = list(set(tb_list))
        df.at[index, 'resultTables'] = res_tb_list
        tb_based_par_res = {}
        for tb in res_tb_list:
            tb_based_par_res[tb] = []
        ...
        for rp in res_partitions:
            ...
            erps.append(partition_manager.get_partition_encoding(rp))
            if separateTbs:
                par_tb = partition_manager.get_partition_table(rp)
                tb_based_par_res[par_tb].append(int(rp.rsplit('_', 1)[1]))


        df.at[index, 'encResultBlock'] = erps
        if separateTbs:
            for tb in tb_based_par_res:
                tb_based_par_res[tb] = sorted(tb_based_par_res[tb])

            df.at[index, 'partitionNumbers_perTb'] = tb_based_par_res
