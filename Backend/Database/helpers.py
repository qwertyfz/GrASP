...
def get_max_block_size(tb_name, db_name=''):
    if db_name == '':
    ...
    sql = f"select max(cnt) from (select count(*) as cnt from {tb_name} group by (ctid::text::point)[0]::bigint/{Config.logical_block_size}) t"
    ...
    if row:
        ...
        print(f'error while getting max block size for table{tb_name}')
