...
class PartitionManager:
    ...
    def put_partition_in_cache(self, p_id, cache: LRUCache, lim=-1, allowed_tbs=[], np_contents=[], check_insertions=False):
        p_block_list_ = self.partitions.get(p_id).blocks
        if len(allowed_tbs) > 0:
            p_block_list = [bid for bid in p_block_list_ if bid.rsplit('_', 1)[0] in allowed_tbs]
        else:
            p_block_list = p_block_list_

        if lim != -1:
            p_block_list = p_block_list[:lim]
        for b_id in p_block_list:
            cache.put(b_id, increase_hit=False, insert_type='p', np_contents=np_contents, check_insertions=check_insertions)
    ...
    def get_partition_table(self, pid):
        if '_' in pid:
            return pid.rsplit('_', 1)[0]
        par = self.partitions.get(pid)
        if par is None:
            return None
        first_bid = par.blocks[0]
        return first_bid.rsplit('_', 1)[0]
